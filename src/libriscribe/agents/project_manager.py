...
class ProjectManagerAgent:
    ...
    def __init__(self, llm_client: LLMClient = None):
        ...
        self.agents = {}
        self.chapter_flow_manager: Optional[ChapterFlowManager] = None
    ...
    def initialize_llm_client(self, llm_provider: str):
        ...
        self.agents = {
            "content_reviewer": ContentReviewerAgent(self.llm_client),
        ...
        # Initialize chapter flow manager if project is already loaded
        if self.project_knowledge_base and self.project_dir:
            self.chapter_flow_manager = ChapterFlowManager(
                llm_client=self.llm_client,
                project_dir=self.project_dir,
                project_knowledge_base=self.project_knowledge_base,
                agents=self.agents
            )
            logger.info("ChapterFlowManager initialized after LLM client setup")
    ...
    def initialize_project_with_data(self, project_data: ProjectKnowledgeBase):
        ...
        # Initialize chapter flow manager
        if self.llm_client and self.agents:
            self.chapter_flow_manager = ChapterFlowManager(
                llm_client=self.llm_client,
                project_dir=self.project_dir,
                project_knowledge_base=self.project_knowledge_base,
    ...
    def write_chapter(self, chapter_number: int):
        ...
        if self.chapter_flow_manager:
            self.chapter_flow_manager.write_chapter(chapter_number)
        else:
    ...
    def write_and_review_chapter(self, chapter_number: int):
        """Write and review chapter using ChapterFlowManager."""
        if not self.chapter_flow_manager:
            console.print("[red]ERROR: Chapter flow manager not initialized[/red]")
            return "next"
        
        result = self.chapter_flow_manager.write_and_review_chapter(chapter_number)
        self.save_project_data()
        return result
    ...
    def edit_chapter(self, chapter_number: int):
        ...
        if self.chapter_flow_manager:
            self.chapter_flow_manager.edit_chapter(chapter_number)
        else:
    ...
    def check_plagiarism(self, chapter_number: int):
        ...
        results = self.agents["plagiarism_checker"].execute(chapter_path)
    ...
    def check_facts(self, chapter_number: int):
        ...
        chapter_path = str(self.project_dir / f"chapter_{chapter_number}.md")
        results = self.agents["fact_checker"].execute(chapter_path)
        print(f"Fact-check results for chapter {chapter_number}: {results}")

    def review_content(self, chapter_number: int):
        """Reviews chapter content using the ReviewManager."""
        if self.chapter_flow_manager:
            return self.chapter_flow_manager.review_chapter(chapter_number)
        else:
            ...
            try:
                ...
                console.print(f"[red]ERROR: Could not read chapter file {chapter_path}: {e}[/red]")
                return {}

            results = self.agents["content_reviewer"].execute(
            ...
            review_md = (
                results.get("review_markdown")
                or results.get("review")
    ...
    def create_title_page(self, project_knowledge_base: ProjectKnowledgeBase) -> str:
        """Creates a Markdown title page."""
        title = project_knowledge_base.title
        author = project_knowledge_base.get('author', 'Unknown Author')
